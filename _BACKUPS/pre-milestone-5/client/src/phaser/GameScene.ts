...
import { fetchGotchiSVGs, removeBackgroundFromSVG } from './FetchGotchis'; // Adjust import path as needed
...
export class GameScene extends Phaser.Scene {
    ...
    // all ui elements are added to this which controls scroll(0), depth and scaling
    ...
    preload() {
        ...
        if (gotchi) {
            ...
            console.error('No selected Gotchi found in registry');
        }
    }
        ...
        this.ws.onmessage = (e) => {
            ...
            switch (msg.type) {
                case "init":
                    ...
                    break;
                case "newPlayer":
                    console.log('New player received:', JSON.stringify(data));
                    this.addPlayer(data);
                    break;
                case "playerDisconnected":
                    console.log('Player disconnected received:', JSON.stringify(data));
    ...
    async loadPendingPlayers() {
        ...
        for (const p of playersToLoad) {
            ...
            const svgDataUrl = `data:image/svg+xml;base64,${btoa(removeBackgroundFromSVG(svgs.front))}`;
